...
mcar_test(ess_df_selected) # Given the high statistic value and low p-value, we can conclude that the data is not missing completely at random.
...
table(missing_rows)
# Rows to exclude:
rows_less50 <- subset(ess_df_selected, missing_rows <= 50) # people whose missing data we can replace
rows_more50<- subset(ess_df_selected, missing_rows > 50) # people who have too much missing data
...
var_names <- names(ess_df_selected_lf)
...
mvn_test <- mvn(data = ess_df_selected_lf, # our data 
...
# 2. Linearity => to be assessed once CFA and SEM models fit
...
fitMeasures(fit_cfa_IPT, c("logl","AIC", "BIC", "chisq", "df", "pvalue", "cfi", "tli","rmsea"), output = "matrix")
...
head(factor_values)
## Trust------------------------------------------------------------------------
trust_df <- as.data.frame(cbind(factor_values$Trust, factor_values$INSTT, factor_values$IP))
trust_df <- trust_df %>% 
  rename("Trust" = "V1" ,
         "INSTT" = "V2",
         "IP" = "V3")
...
ggpairs(trust_df, aes(color = as.factor(ess_df_selected$gndr), alpha = 0.5), lower = list(continuous = "smooth"))

pairs(trust_df)

## Sat CSA -------------------------------------------------------------------------------

sat_csa_df <- as.data.frame(cbind(factor_values$SatCSA, ess_df_selected$stfeco,
                                     ess_df_selected$stfgov, ess_df_selected$stfdem,
                                     ess_df_selected$stfedu, ess_df_selected$stfhlth))
sat_csa_df <- sat_csa_df %>% 
  rename("Sat_CSA" = "V1" ,
         "stfeco" = "V2",
         "stfgov" = "V3",
         "stfdem" = "V4",
...
ggpairs(sat_csa_df, aes(color = as.factor(ess_df_selected$gndr), alpha = 0.5), lower = list(continuous = "smooth"))

pairs(sat_csa_df)

## IP --------------------------------------------------------------------------

IP_df <- as.data.frame(cbind(factor_values$IP, ess_df_selected$ppltrst,ess_df_selected$pplhlp, ess_df_selected$pplfair))
IP_df <- IP_df %>% 
  rename("IP" = "V1" ,
         "ppltrst" = "V2",
         "pplhlp" = "V3",
...
ggpairs(IP_df, aes(color = as.factor(ess_df_selected$gndr), alpha = 0.5), lower = list(continuous = "smooth"))

pairs(IP_df)

## ISTT --------------------------------------------------------------------------

INSTT_df <- as.data.frame(cbind(factor_values$INSTT, ess_df_selected$trstlgl,ess_df_selected$trstplc, ess_df_selected$trstplt,
                                ess_df_selected$trstprl,ess_df_selected$trstep, ess_df_selected$trstun,
                                ess_df_selected$trstprt))
INSTT_df <- INSTT_df %>% 
  rename("INSTT" = "V1" ,
         "trstlgl" = "V2",
         "trstplc" = "V3",
...
ggpairs(INSTT_df, aes(color = as.factor(ess_df_selected$gndr), alpha = 0.5), lower = list(continuous = "smooth"))

pairs(INSTT_df)

## PolEf ---------------------------------------------------------------------------

PolEf_df <- as.data.frame(cbind(factor_values$PolEf, ess_df_selected$actrolg,
                                ess_df_selected$cptppol, ess_df_selected$psppipl,
                                ess_df_selected$psppsgv,ess_df_selected$ptcpplt))
PolEf_df <- PolEf_df %>% 
  rename("PolEf" = "V1" ,
         "actrolg" = "V2",
         "cptppol" = "V3",
...
ggpairs(PolEf_df, aes(color = as.factor(ess_df_selected$gndr), alpha = 0.5), lower = list(continuous = "smooth"))
...
# effect other latent vars than the ones we associate them. 
# Fit usually gets worse and worse, the more
...
# Visualization
semPaths(fit_mimic_fiml, what = "std", optimizeLatRes = TRUE, layout = "spring", style = "ram", nCharNodes = 0, intercepts = FALSE, fade = FALSE)
...
ptcpplt ~ lrscale
'
fit_mimic_fiml_e <- sem(model_mimic_e,             
                      estimator = "ML",
                      missing = 'direct',
                      data = ess_df_selected
)

summary(fit_mimic_fiml_e, 
        fit.measures = TRUE,   
        standardized = TRUE 
)

fitm_mimic_e <- fitMeasures(fit_mimic_fiml_e, c("logl","AIC", "BIC", "chisq", "df", "pvalue", "cfi", "tli","rmsea"), output = "matrix")
fitm_mimic_e

## Modification Indexes
mi <- inspect(fit_mimic_fiml_e,"mi")
mi.sorted <- mi[order(-mi$mi),] # sort from high to low mi.sorted[1:5,] # only display some large MI values
mi.sorted[1:5,] # only display some large MI values

# Visualization
semPaths(fit_mimic_fiml_e, what = "std", optimizeLatRes = TRUE, layout = "spring", style = "ram", nCharNodes = 0, intercepts = FALSE, fade = FALSE)
...
# Visualization
semPaths(fit_mimic2, what = "std", optimizeLatRes = TRUE, layout = "spring", style = "ram", nCharNodes = 0, intercepts = FALSE, fade = FALSE)
...
# Visualization
semPaths(fit_mimic3, what = "std", optimizeLatRes = TRUE, layout = "spring", style = "ram", nCharNodes = 0, intercepts = FALSE, fade = FALSE)
...
# Visualization
semPaths(fit_mimic4, what = "std", optimizeLatRes = TRUE, layout = "spring", style = "ram", nCharNodes = 0, intercepts = FALSE, fade = FALSE, rotation = 2)
...
# Visualization
semPaths(fit_mimic5, what = "std", optimizeLatRes = TRUE, layout = "spring", style = "ram", nCharNodes = 0, intercepts = FALSE, fade = FALSE)
...
# MIMIC
PolEf ~ hinctnta + agea + edlvdhu + fclcntr + gndr
'

fit_mimic_full <- sem(model_mimic_full,             
                      estimator = "ML",
                      missing = 'direct',
                      data = ess_df_selected
)

summary(fit_mimic_full, 
        fit.measures = TRUE,   
        standardized = TRUE 
)

fitm_mimic_full <- fitMeasures(fit_mimic_full, c("logl","AIC", "BIC", "chisq", "df", "pvalue", "cfi", "tli","rmsea"), output = "matrix")
fitm_mimic_full

## Modification Indexes
mi <- inspect(fit_mimic_full,"mi")
mi.sorted <- mi[order(-mi$mi),] # sort from high to low mi.sorted[1:5,] # only display some large MI values
mi.sorted[1:5,] # only display some large MI values

# Visualization
semPaths(fit_mimic_full, what = "std", optimizeLatRes = TRUE, layout = "spring", style = "ram", nCharNodes = 0, intercepts = FALSE, fade = FALSE, rotation = 2)

######################################################################################

data.frame(
           "w/lrscale" = round(fitm_mimic[,1],2),
           "w/hinctnta" = round(fitm_mimic_e[,1],2),
           ...
           "w/hinctnta + agea + eduhl + fclcntr + gndr" = round(fitm_mimic_full[,1],2)
)
...
# Visualization
semPaths(fit_mediation_fiml, what = "std", optimizeLatRes = TRUE, layout = "tree", nCharNodes = 0, intercepts = FALSE, fade = FALSE)
...
# Visualization
semPaths(fit_mediation3, what = "std", optimizeLatRes = TRUE, layout = "tree", style = "ram", nCharNodes = 0, intercepts = FALSE, fade = FALSE, rotation = 2)
...
fit_mediation4 <- cfa(model_mediation4, 
                      estimator = "ML",
                      missing = 'direct',
                      data = ess_df_selected
)

summary(fit_mediation4, 
        fit.measures = TRUE,   
        standardized = TRUE 
)
...
# Visualization
semPaths(fit_mediation4, what = "std", optimizeLatRes = TRUE, sizeMan2 = 2, layout = "tree", style = "Lisrel", nCharNodes = 0, intercepts = FALSE, fade = FALSE, rotation = 2)


# Global fit
fitMeasures(fit_mediation4, c("logl","AIC", "BIC", "chisq", "df", "pvalue", "cfi", "tli","rmsea"), output = "matrix")
# Local fit (Modification Indexes)
mi <- inspect(fit_mediation4,"mi")
mi.sorted <- mi[order(-mi$mi),] # sort from high to low mi.sorted[1:5,]
mi.sorted[1:5,] # only display some large MI values
...
semPaths(fit_structural, optimizeLatRes = TRUE, layout = "tree", style = "ram", nCharNodes = 0, intercepts = FALSE, fade = FALSE)
...
# Anova uses X^2 => sensitive to departures from normality and large sample sizes => detects small deviations
...
table_fit <- 
  list(model_fit(fit_metric),
       model_fit(fit_scalar), 
       model_fit(fit_scalar_psppsgv), 
       model_fit(fit_scalar_psppsgv_ptcpplt), 
       model_fit(fit_scalar_psppsgv_ptcpplt_psppipl)) %>% 
  reduce(rbind)


rownames(table_fit) <- c("Metric", "Scalar", "Plus_psppsgv","Plus_ptcpplt","Plus_psppipl")
...
PolEf~ c("c_fclcntr_1", "c_fclcntr_2")*fclcntr
PolEf ~ c("c_po_1", "c_po_2")*lrscale

## Mediator ##
# Path A
Trust ~ c("a_edu_1", "a_edu_2")*edlvdhu
Trust ~ c("a_inc_1", "a_inc_2")*hinctnta
Trust~ c("a_fclcntr_1", "a_fclcntr_2")*fclcntr
Trust ~ c("a_po_1", "a_po_2")*lrscale
...
ab_edu_F := a_edu_1*b1
ab_inc_F := a_inc_1*b1
ab_fclcntr_F := a_fclcntr_1*b1
ab_po_F := a_po_1*b1

# M 
ab_edu_M := a_edu_2*b2
ab_inc_M := a_inc_2*b2
ab_fclcntr_M := a_fclcntr_2*b2
ab_po_M := a_po_2*b2

## Total effect c + (a*b) ##
# F 
total_edu_F := c_edu_1 + (a_edu_1*b1)
total_inc_F := c_inc_1 + (a_inc_1*b1)
total_fclcntr_F := c_fclcntr_1 + (a_fclcntr_1*b1)
total_po_F := c_po_1 + (a_po_1*b1)

# M 
total_edu_M := c_edu_2 + (a_edu_2*b2)
total_inc_M := c_inc_2 + (a_inc_2*b2)
total_fclcntr_M := c_fclcntr_2 + (a_fclcntr_2*b2)
total_po_M := c_po_2 + (a_po_2*b2)
'
...
## Direct effect ##
PolEf ~ c("c_edu_1", "c_edu_1")*edlvdhu
PolEf ~ c("c_inc_1", "c_inc_1")*hinctnta
PolEf~ c("c_fclcntr_1", "c_fclcntr_1")*fclcntr
PolEf ~ c("c_po_1", "c_po_1")*lrscale

## Mediator ##
# Path A
Trust ~ c("a_edu_1", "a_edu_1")*edlvdhu
Trust ~ c("a_inc_1", "a_inc_1")*hinctnta
Trust~ c("a_fclcntr_1", "a_fclcntr_1")*fclcntr
Trust ~ c("a_po_1", "a_po_1")*lrscale
...
table_fit <- 
  list(model_fit(fit_mediation_mg),
       model_fit(fit_mediation_mg_cons)) %>% 
  reduce(rbind)

rownames(table_fit) <- c("Unconstrained", "Constrained")
...
detach("package:naniar", unload = T)
